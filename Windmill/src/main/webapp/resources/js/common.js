...
function sendSql(value) {
	...
	for (var i = 0; i < column.length; i++) {
		...
		str += $(".Resultrow.success").children('td').eq(column[i]).html();
	}
	...
	if (value.includes("FileRead")) {
		...
		for (var i = 0; i < column.length; i++) {
			...
			if (column[i].match(nCheck)) {
				pathval += $(".Resultrow.success").children('td').eq(column[i]).html();
			} else {
				pathval += column[i];
			}
		}
		...
		for (var i = 0; i < column.length; i++) {
			if (column[i].match(/^\d{1,2}$/)) {
				...
				for (var j = 0; j < $(".Resultrow").length; j++) {//$(".Resultrow").length
					pathval += $(".Resultrow").eq(j).children('td').eq(column[i].substr(0, column[i].length-1)).html()+"/";
				}
			} else {
				pathval += column[i];
			}
