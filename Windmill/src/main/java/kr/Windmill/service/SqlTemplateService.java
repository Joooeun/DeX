...
public class SqlTemplateService {
	...
	public Map<String, Object> getTemplateShortcuts(String templateId) {
		...
		try {
			String sql = "SELECT SHORTCUT_KEY, SHORTCUT_NAME, TARGET_TEMPLATE_ID, SHORTCUT_DESCRIPTION, SOURCE_COLUMN_INDEXES, AUTO_EXECUTE, IS_ACTIVE FROM SQL_TEMPLATE_SHORTCUT WHERE SOURCE_TEMPLATE_ID = ? ORDER BY CAST(SUBSTRING(SHORTCUT_KEY FROM 2) AS INTEGER)";
